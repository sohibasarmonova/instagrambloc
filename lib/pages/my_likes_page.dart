...
late MyLikedPostBloc myLikedBloc;

@override
void initState() {
  super.initState();
  myLikedBloc= context.read<MyLikedPostBloc>();
  myLikedBloc.add(LoadLikedPostsEvent());
}
@override
Widget build(BuildContext context) {
  return BlocConsumer<MyLikedPostBloc, MyLikedState>(
...
@override
  ...
  Widget _itemOfPost(Post post) {
    return BlocProvider(create: (context)=>MyLikedPostBloc(),
      child: BlocBuilder<MyLikedPostBloc,MyLikedState>(
        builder: (context,state){
          return Container(
            color: Colors.white,
            child: Column(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      post.mine
                        ...
                        onPressed: () {
                          context.read<MyLikedPostBloc>().add(RemovePostEvent( post: post));
                        },
                      )
                          : const SizedBox.shrink(),
                    ],
                  ...
                  children: [
                    Row(
                      children: [
                        IconButton(
                          onPressed: () {
                            myLikedBloc.add(UnLikePostEvent(post: post));
                            // _apiPostUnLike(post);
                          },
                          icon: post.liked
                              ? const Icon(
